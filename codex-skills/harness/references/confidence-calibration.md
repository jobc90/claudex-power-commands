...
- **Review mode only — capture-then-filter (REPORT surface, not FIX)**: In review-discovery (Analyzer), CAPTURE every candidate >= 60 in a first pass, THEN apply the >= 80 report threshold as a SEPARATE second pass. Demote 60-79 candidates to the existing labeled "Below 70 / Deferred / Recommendations for QA" sink — never silently drop them. This rule governs only the **REPORT surface** in review mode; it does NOT touch FIX decisions. The Key Principle above (probably/might/seems -> below 80, don't report/fix) still governs every **FIX** decision, and all Fix thresholds in the Action Thresholds table stay unchanged.
