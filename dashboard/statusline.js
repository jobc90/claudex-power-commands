...
async function fetchUsageLimits(ttlSeconds = 60) {
  ...
  if (!token) {
    if (lastTokenHash) {
      ...
      if (fileCache2) return fileCache2;
    }
    return null;
  }
  ...
  try {
    ...
    const expiredMemory = usageCacheMap.get(tokenHash);
    if (expiredMemory) return expiredMemory.data;
    const expiredFile = await loadFileCache(tokenHash, 86400);
    if (expiredFile) return expiredFile;
    return null;
  } finally {
...
var OAUTH_CLIENT_ID = "REDACTED_OAUTH_CLIENT_ID";
var OAUTH_CLIENT_SECRET = "REDACTED_OAUTH_CLIENT_SECRET";
...
async function getGeminiSettings() {
  try {
    ...
    const data = {
      ...
      selectedModel: json?.selectedModel || (typeof json?.model === "object" ? json?.model?.name : json?.model),
      auth: json?.auth
    };
