...
>
> **v4.6.0**: Frontier refresh — the tier fallback now recognizes `fable`/`mythos`/`sol` as **Elite** (previously a Fable 5 session suffered a **tier inversion** down to Standard; the unknown-model default also moves Standard→Advanced). Elite-parent model economy: labor agents get `model: "opus"`, only the three judgment roles (Planner/Architect/Diagnostician) keep inheriting; the Codex runtime always inherits (translation table in `codex-skills/AGENTS.md`). Corrects v4.5.1's byte-parity claim — 9 reference mirrors had drifted; all re-synced, and the pre-commit lint now diffs shared references dynamically. `.harness_codex` unified to `.harness/`. Golden baseline is sonnet-era: explicitly marked **re-baseline pending**.
> **v4.5.1**: All harness subagents **inherit the parent session model** — the per-role sonnet/haiku downgrade table is retired, so the session model (`/effort`, `/model`) is the single control knob (an Opus session runs every agent on Opus; tier-detection is unaffected). Trade-off: running from a non-Opus session lets subagents follow it down.
