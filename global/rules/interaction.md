...
| 순위 | 도구 | 용도 |
|------|------|------|
| 1 | `mcp__playwright__*` | JS 렌더링·상호작용이 필요한 페이지, 영상·문서 페이지의 텍스트 추출 |
| 2 | WebFetch | 단순 정적 페이지. 지연되면 즉시 물러난다 |

설치되지 않은 도구를 규칙으로 강제하지 마라. 과거 이 규칙은
`mcp__jina-reader__`·`mcp__fetch__`를 필수로 지정했으나 둘 다 미설치라,
결과적으로 작동하는 유일한 도구를 금지했다. 같은 이유로 2026-08-18
`summarize` 스킬을 1순위에서 제거했다 — binary 미설치·API 키 없음·gemini
기본값 stale·사용 이력 0 (스킬은 `global/attic/commands/summarize/` 보관).
도구를 추가하려면 먼저 설치하고 동작을 확인한 뒤 이 표에 올린다.
