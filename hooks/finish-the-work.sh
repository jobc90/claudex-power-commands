#!/bin/bash
# finish-the-work.sh — Stop hook (claudex)
# claudex's first RUNTIME-enforced completion entrance. The Meta-Loop / Phase Verifier /
# Completion Gate are agent-self-enforced (soft entrance — see meta-loop-protocol.md §1
# "Enforcement boundary"); this hook is the runtime backstop the agent cannot opt out of.
# It detects early termination where the agent only PROMISES work without doing it, and re-engages it.
...
# Origin: ported from fivetaku/fablize (MIT) hooks/finish-the-work.sh. Effect on claudex's
# model mix is not yet A/B-measured.
